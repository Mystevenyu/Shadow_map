...
namespace labutils
{
	...
	Sampler create_shadow_sampler(VulkanContext const& aContext)
	{
		...
		shadowSamplerInfo.maxLod = VK_LOD_CLAMP_NONE;
		shadowSamplerInfo.compareEnable = VK_TRUE; // Enable comparison for shadow mapping
		shadowSamplerInfo.compareOp = VK_COMPARE_OP_LESS; // Pixel passes if it's in front of the light

		VkSampler shadowSampler = VK_NULL_HANDLE;
		if (auto const res = vkCreateSampler(aContext.device, &shadowSamplerInfo, nullptr, &shadowSampler);
			VK_SUCCESS != res)
		{
			throw Error("Unable to create shadow sampler\n"
				"vkCreateSampler() returned %s", to_string(res).c_str());
		}

		return Sampler(aContext.device, shadowSampler);
	}
